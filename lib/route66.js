...
Route66.addRoute = function(method, route, functions) {
  ...
  Route66.routes[method].push({
    route: route,
    regex: new RegExp('^' + route.replace(/\//g, '\\/').replace(/\:([A-Za-z_]+)(\?)?\/?/g, '([A-Za-z0-9._-]+)$2') + '\\/?$'),
    params: params,
...
Route66.sort = function() {
  ...
  sort = function(a, b) {
    ...
    if (-1 < aRegex.indexOf('([A-Za-z0-9_-]+)')) {
      ...
      return bRegex.length - aRegex.length;
    }
  };
  for (_i = 0, _len = methods.length; _i < _len; _i++) {
    ...
    if (Route66.routes[method].length > 0) {
      ...
      for (_j = 0, _len1 = _ref.length; _j < _len1; _j++) {
        ...
        if (route.route === '/') {
          last.push(route);
        } else if (!/[A-Za-z0-9_]+/.test(route.route.replace(/\:[A-Za-z0-9_]+/g, ''))) {
          middle.push(route);
        } else {
          first.push(route);
        }
