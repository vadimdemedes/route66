...
class Router {
  ...
  resource (name, options = {}, routes) {
    ...
    if (only.length) {
      methods = methods.filter(method => includes(only, method));
    }
    
    if (except.length) {
      methods = methods.filter(method => !includes(except, method));
    }
  ...
  express () {
    ...
    return function (req, res, next) {
      ...
      let result = router.resolve(req.method, req.url);
  ...
  koa () {
    ...
    return function *(next) {
      ...
      let route = router.resolve(this.method, this.url);
