...
class Router {
  ...
  express () {
    ...
    return function (req, res, next) {
      ...
      let routes = router._compiledRoutes[method];
      ...
      while (route = routes[index++]) {
        ...
        if (params = route.regexp.exec(req.url)) {
          req.params = params.slice(1).map(param => decodeURIComponent(param));
          route.keys.forEach((key, i) => req.params[key] = req.params[i]);
  ...
  koa () {
    ...
    return function *(next) {
      ...
      while (route = routes[index++]) {
        ...
        if (params = route.regexp.exec(this.url)) {
          this.params = params.slice(1).map(param => decodeURIComponent(param));
          route.keys.forEach((key, i) => this.params[key] = this.params[i]);
